...
const Create = ({ open, onClose }) => {
                    ...
                    const handleCountryChange = (event) => {
                        ...
                        fetch(`https://vinoapi.winayak.com/api/core/states?country_id=${selectedCountry.code}`)
                        .then(response => response.json())
                        .then(data => {
                            console.log('Fetched states:', data); // Log the fetched states
                            setStates(data.list || []); // Set states list or default to empty array
                        })
                        .catch(error => {
                    ...
                    const handleAddUser = () => {
                        ...
                        const newUser = {id:Date.now(), name, email, mobile, state, country };
                ...
                <FormControl fullWidth margin="normal">
                    ...
                    >
                        {states.length > 0 && states.map((state) => (
                            <MenuItem key={state.id} value={state.code}> {/* Ensure `state.code` is unique */}
                                {state.name}
                            </MenuItem>
                        ))}
                    </Select>
